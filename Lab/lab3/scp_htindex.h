...
typedef scp_htindex_data_s     scp_htindex_data_t;
typedef scp_htindex_item_s     scp_htindex_item_t;
typedef scp_stk_order_s        scp_stk_order_t;
typedef scp_bpu_s              scp_bpu_t;
...
{
	kut_htindex_data_t	    *start; //缓冲区启始地址
	kut_htindex_data_t	    *last;  //缓冲区未使用的ordersn的首地址
	kut_htindex_data_t		*end;   //缓冲区最后一项地址
...
#define SCP_BPU_NUM           		4
#define SCP_INDEX_ITEM_NUM    		4
#define SCP_INDEX_ORDERSN_NUM 	    1024

//应放在main.cpp中定义
scp_bpu_t    *scp_bpus = NULL;

...
static scp_table_item_t *scp_table_item_create(scp_table_t *table)
{
	...
	//实现剩余代码
	item->start = (u_char*)item + sizeof(scp_table_item_t);
	item->last = item->start;
	item->end = item->start + (table->size * table->n);
	item->data = table;
	return item;
}
...
void *scp_table_add_handler(scp_table_t *table, scp_index_handle sindex, scp_open_hash_key_pointer_t key, void *buf, int len)
{
	...
	{
		data = item->last;
		memcpy(item->last, buf, len);
		item->last = item->last+len;
	} 
	scp_open_hash_t* hash = (scp_open_hash_t*)sindex;
	hash->add_handler(hash, key, data);
	return data;
}
...
void *scp_table_get_handler(scp_table_t *table, scp_index_handle sindex, scp_open_hash_key_pointer_t key)
{
	...
	//实现代码	
	scp_open_hash_t* hash = (scp_open_hash_t*)sindex;
	item = (scp_open_hash_item_t*)hash->find_handler(hash, key);
	if (!item) 
		return NULL;
	return 
		item->value;
